...
### 🎯 다음 세션 메인 태스크: 클로바 OCR 갑구 파서를 실제 라우터에 배선

독립 검증(위 "클로바 OCR 실키 검증" 섹션)이 끝난 클로바 OCR 경로를 실제 서비스에
공식 연결하는 작업. 2026-09-15 밤 세션에서 다음 세션 계획으로 합의된 내용:

1. **`POST /registry/upload` 개조** — 지금은 요약 페이지(`registry_summary_ocr.py`)만
   파싱해 미리보기를 주는데, 여기에 클로바 OCR 어댑터(`clova_ocr_adapter.py`)와
   방금 갭을 고친 갑구 파서(`gapgu_ocr_row_parser.py`)를 정식 연결한다. (참고: 클로바는
   이미지 해상도 상한이 8000px라 페이지를 통째로 큰 DPI로 렌더링하면 안 됨 — 이번
   세션에서 300 DPI가 넘어서 `ERROR`가 났던 걸 200 DPI로 낮춰 해결했던 경험 참고)
2. **`parse_gapgu()` 연동 + 사기 패턴 최종 조율** — `extract_rows_from_clova_result()`가
   재조합한 rank/purpose/receipt/cause/detail 딕셔너리 리스트를 이미 완성된
   `registry_parser.parse_gapgu()`로 흘려보내고, 그 결과(`ownershipHistory`)가
   `fraud_pattern_rules.py`(신축빌라+소유주변경 패턴)까지 실제로 타는지 엔드투엔드로
   확인한다 — 지금은 이 이력 데이터 부재를 "판별 불가"로만 처리하고 있어서, 실제
   데이터가 들어왔을 때 패턴 판별 자체가 의도대로 동작하는지가 이 작업의 핵심.
3. 이번 세션에서 발견한 "표제부가 같은 이미지에 섞여 있으면 노이즈 행이 낀다"(위
   "클로바 OCR 실키 검증" 섹션 참고)는 실사용 아파트 갑구에서는 거의 안 생길 거라
   예상했지만, 실제 배선하면서 다시 한번 확인할 것 — 노이즈 행의 `purpose`가 항상
   빈 문자열이라 `parse_gapgu()`가 자동으로 거르는 것까지는 확인됐지만, 실제 엔드투엔드
   응답에서도 그대로 안전한지 재검증 필요.
