...
1. **을구 자동 데이터 추출은 아직 없음** — `registry_critical_keywords`(임차권등기명령 등
   갑구/을구 치명적 키워드)와 을구 근저당권 채권최고액 합계를 이번 클로바 경로로 자동
   추출하는 건 이번 세션 스코프 밖으로 남겨뒀다. 지금은 근저당/전세권 합계가 요약
   페이지(`registry_summary_parser.py`)로 이미 커버되고 있어 급하지 않지만, 말소되지
   않은 오래된 근저당(요약 페이지엔 안 잡히는 케이스가 있는지)까지 완전히 커버하려면
   `registry_parser.parse_eulgu()`도 같은 방식(모든 페이지 → 클로바 → 파서)으로 배선할
   필요가 있는지 검토할 것.
