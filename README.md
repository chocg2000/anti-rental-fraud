...
## 📍 현재 상태 요약 (2026-09-16 밤 기준 — 다음 세션은 여기부터 읽을 것)

**B2C 무료 버전 출시가 목표.** 핵심 진단 파이프라인은 완성/검증됐다(아래 "완료된
모듈" 표). 이번 세션(하루 종일 이어짐)에 추가로 끝낸 것:

- 을구(근저당/임차권등기명령) 파이프라인 실키 검증 + 배선, 그 과정에서 발견한 행
  파서 회귀 버그 수정 — "을구 파이프라인 배선 + 행 파서 재설계" 섹션
- 사기 패턴 위험 시각화 UI(Step3) 구현 + 실제 브라우저로 검증 완료
- 골든 패스(Step1→2→3) 종단 테스트 완료
- 배포 직전 환경변수 점검 완료, SQLite 백업은 지금 단계에서 리스크 감수하기로 결정
- **OWASP 보안 점검**: nginx 업로드 크기 미스매치, rate limiting 부재, IP 위조를 통한
  rate limit 우회, backend 포트 호스트 노출까지 전부 발견하고 수정 — "OWASP Top 10
  기준 보안 점검" 섹션
- 면책 문구(Step3 고정 배너) + 개인정보 수집·이용 동의 체크박스(Step1, **문구는
  placeholder**) 추가
- SEO/AEO/GEO: 랜딩 페이지(`/`) 신설(5대 사기유형/FAQ/사용법 가이드) + FAQPage·HowTo
  구조화 데이터 + `robots.txt`/`sitemap.xml` — **도메인이 아직 `https://example.com`
  placeholder**

### 🔜 다음 세션 시작 전, 유저가 직접 확인하기로 한 것

1. **개인정보처리방침 실제 문구** (법률 검토 필요) — `frontend/src/steps/Step1Address.jsx`의
   자리표시자 교체
2. **HTTPS 방식 선택** — Cloudflare "Full (strict)" vs nginx+certbot 직접 설정
   ("B2C 출시 전 최종 점검 — 법적/인프라 방어벽" 섹션에 두 옵션 정리돼 있음)
3. **에러 모니터링 필요 여부** — 지금은 콘솔 로그(`logger.exception`)뿐, Sentry 등
   도입 여부 결정 필요

이 세 가지가 정해지면 그에 맞춰 코드에 반영할 것. 그 전까지 별도 지시가 없으면
을구 커버리지 재점검이나 B2B 준비 항목(클로바 호출 병렬화 등)으로 진행 가능 —
"다음 세션 후보 태스크" 섹션 참고. 배포 도메인이 정해지면 `index.html`/
`robots.txt`/`sitemap.xml`의 `https://example.com`도 같이 교체할 것.

**현재 상태**: 백엔드 테스트 326개 전부 통과, 프론트 `lint`/`build` 클린, git 전부
커밋됨(마지막 커밋 `c630b10`), 워킹 트리 깨끗함.
...
### ⚠️ B2C 출시 전 최종 점검 — 법적/인프라 방어벽 (2026-09-16 밤)

OWASP 점검(위 섹션)을 코드 관점에서 마친 뒤, "B2C 출시에 더 체크할 게 없는지" 다시
훑다가 코드 리뷰만으로는 안 보이는 두 가지를 새로 찾았다 — 개인정보(주소·보증금·
임대인 이름·등기부 PDF)를 다루는 공개 서비스인데 정작 (1) 전송 구간 암호화(HTTPS)와
(2) 개인정보 수집·이용 동의/면책 고지가 전혀 없었다.

1. **면책 문구 추가 (완료)** — `Step3Result.jsx` 결과 화면 맨 아래, 아코디언 없이
   항상 펼쳐진 상태로 고정 배너 추가: "본 진단 결과는 공공 데이터 기반 참고용이며
   법적 효력이 없다", "전문가와 상의하고 본 서비스는 재산상 손실에 책임지지 않는다",
   "데이터 수집 시점과 실제 계약 시점의 시차로 차이가 발생할 수 있다" 3개 문구.
   유저가 "안전이라고 해서 계약했다가 사기당했다"며 책임을 물을 때의 최소한의
   방어선 — 다만 이건 면책 "고지"일 뿐 법적 자문을 대체하지 않는다.
2. **개인정보 수집·이용 동의 체크박스 추가 (완료, 문구는 자리표시자)** —
   `Step1Address.jsx`에 필수 체크박스 추가, 체크 안 하면 다음 단계로 못 넘어감
   (`App.jsx`의 `isStep1Complete`도 동일하게 가드). **문구 자체는 자리표시자
   (placeholder)로 남겨뒀다** — 실제 수집 항목/목적/보유기간/제3자 제공 여부(예:
   카카오/국토부/VWorld/클로바에 주소·PDF가 전달된다는 사실 고지 포함)를 반영한
   정식 개인정보처리방침으로 반드시 교체해야 한다. 이 자리표시자 상태로 실서비스에
   배포하지 말 것 — 코드에도 동일한 경고 주석을 남겨둠.
3. **HTTPS/TLS — 아직 미해결, 다음에 결정할 것.** `frontend/nginx.conf`는 80번
   포트(평문 HTTP)만 연다. 등기부 PDF(정부 문서 자체가 이미 부분마스킹한
   주민등록번호 포함)·주소·보증금을 평문으로 주고받는 건 실제 공개 서비스로는
   받아들이기 어렵다. 두 가지 선택지가 있고 코드는 아직 안 건드렸다:
   - **(a) CDN/리버스 프록시에서 TLS 종료** — Cloudflare를 백엔드/프론트 앞에 두고
     "Flexible" 대신 반드시 "Full (strict)" 모드로 설정(안 그러면 Cloudflare↔서버
     구간은 여전히 평문). 인프라 쪽에 별도 인증서 발급/갱신 관리가 필요 없어 가장
     간단하다.
   - **(b) nginx에서 직접 TLS 종료** — Let's Encrypt(certbot)로 인증서를 받아
     `frontend/nginx.conf`의 `listen 80` 옆에 `listen 443 ssl;` 서버 블록을 추가하고
     80번은 443으로 리다이렉트. 도메인이 이 서버를 직접 가리켜야 하고, 인증서
     자동 갱신(certbot 컨테이너/크론)을 직접 운영해야 한다.
   실제 배포 도메인/인프라(어떤 클라우드에 올릴지, CDN을 쓸지)가 정해져야 (a)/(b)
   중 뭐가 맞는지 정할 수 있어서 이번엔 가이드만 남기고 코드는 안 건드렸다.

### ✅ SEO/AEO/GEO 기반 작업 — 랜딩 페이지 신설 (2026-09-16 밤)

지금까지 `/`는 바로 `/step1`(입력 폼)으로 리다이렉트돼서, 검색엔진이나 생성형 AI가
읽을 실질적인 텍스트 콘텐츠 자체가 없었다 — 태그만 넣는다고 SEO/AEO/GEO가 되는 게
아니라, 인용할 만한 정보성 콘텐츠가 먼저 있어야 한다는 전제로 작업했다.

- **`frontend/src/pages/Landing.jsx` 신설**, `/` 라우트를 여기로 교체(기존 리다이렉트
  제거). 4개 섹션 — ① 5대 전세사기 유형과 검증 범위(항목마다 "자동 확인" 여부를
  정직하게 명시 — 위반건축물은 정부 API가 비공개라 자가진단 필요하다고 그대로 밝힘),
  ② FAQ(등기부 을구 "말소" 뜻, 주요등기사항요약 신뢰 여부 등 실제 롱테일 검색어를
  질문 형태 그대로 헤더로 사용), ③ 3단계 사용법 가이드, ④ 타겟 유즈케이스 목록.
  톤은 "~인 것 같다" 같은 애매한 표현 없이 팩트/수치 중심(국토부·대법원 등기부 등
  출처 명시)으로 작성 — AEO/GEO 엔진이 인용할 때 신뢰할 수 있는 출처로 판단하게
  하기 위함.
- **`frontend/src/lib/seo.js` 신설** — SPA라 라우트 이동해도 `<head>`가 안 바뀌는
  문제를 다루는 훅 2개. `useNoIndex()`는 Step1/Step2/Result(개인 입력값·진단 결과가
  담긴 페이지, 검색 노출되면 안 됨)에 적용 — robots.txt로 크롤링 자체를 막지 않고
  `<meta name="robots" content="noindex">`만 쓴 이유는, disallow로 막으면 크롤러가
  noindex 지시 자체를 볼 기회가 없어져 다른 곳 링크만으로도 스니펫 없이 색인될 수
  있다는 구글 공식 가이드 때문. `useJsonLd()`는 FAQPage/HowTo 구조화 데이터를 실제
  화면 콘텐츠와 나란히(같은 컴포넌트 안에서) 관리 — schema.org 가이드라인상 구조화
  데이터는 반드시 화면에 보이는 내용과 일치해야 한다.
- **`index.html`**: description/OG/Twitter 카드 메타 + `WebApplication` JSON-LD
  추가. ⚠️ `og:url`/canonical에 `https://example.com` 플레이스홀더를 넣어뒀다 —
  배포 도메인 확정되면 반드시 교체. `og:image`(소셜 공유 미리보기 이미지)도 아직 없음.
- **`frontend/public/robots.txt` / `sitemap.xml` 신설** — sitemap도 같은 이유로
  `/step1`·`/step2`·`/result/:id`는 의도적으로 제외하고 `/`만 등재. 여기도 도메인
  플레이스홀더 교체 필요.

`npm run lint`/`npm run build` 통과, dev 서버로 `/`와 `/robots.txt` 응답 확인(정적
셸 레벨 스모크 테스트만 — 실제 브라우저 렌더링 확인은 못 함, 이 개발 환경엔 브라우저
자동화 도구가 없음).
